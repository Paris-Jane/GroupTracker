...
export default function TaskFormModal({ task, members, onSave, onClose, onDelete, defaultsForNew }: TaskFormModalProps) {
  ...
  const savePayload = (): CreateTaskDto & { assigneeIds: number[]; subtaskNames: string[] } => ({
    name,
    notes,
    estimatedTime,
    ...
    <div className="modal-overlay">
      <div className="modal modal-lg">
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <MemberSelector members={members} selected={assigneeIds} onChange={setAssigneeIds} compact />
          </div>

          <div className="task-form-advanced">
            <button
              type="button"
              className="task-form-advanced-toggle"
              onClick={() => setAdvancedOpen(o => !o)}
              aria-expanded={advancedOpen}
            >
              Advanced settings
              <span className="task-form-advanced-chevron">{advancedOpen ? '▾' : '▸'}</span>
            </button>
            {advancedOpen && (
              <div className="task-form-advanced-body">
